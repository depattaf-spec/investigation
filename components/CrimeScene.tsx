...
export const CrimeScene: React.FC<CrimeSceneProps> = ({ evidence, onCollectEvidence }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-[#0a101a] text-[#6495ED] font-mono overflow-hidden">
      {/* Blueprint Header */}
      <div className="p-4 border-b border-[#6495ED]/30 flex flex-col md:flex-row justify-between md:items-end bg-[#0a101a] z-10 shrink-0 gap-2 md:gap-0 shadow-lg relative">
      ...
      {/* Blueprint Container */}
      <div className="relative flex-grow bg-[#1a2332] overflow-hidden border-b-4 border-[#2a3b55] cursor-crosshair group shadow-inner">
        {/* Grid Background */}
      ...
      <div className="h-1/3 md:h-[30%] bg-[#0c121d] p-4 overflow-y-auto shrink-0 shadow-[inset_0_10px_30px_rgba(0,0,0,0.8)] border-t border-[#6495ED]/20 relative">
        <div className="sticky top-0 bg-[#0c121d]/95 backdrop-blur-sm pb-3 mb-2 border-b border-[#6495ED]/20 flex items-center justify-between z-20">
            ...
            <span className="text-[10px] font-bold opacity-50 border border-[#6495ED]/30 px-2 py-1 rounded">
               {collectedEvidence.length} / {sceneItems.length} ITEMS
            </span>
        </div>
        
        <div className="space-y-4 pb-4">
            {collectedEvidence.length === 0 ? (
               <div className="h-full flex flex-col items-center justify-center opacity-30 mt-8 space-y-2">
                 <div className="w-12 h-12 border-2 border-dashed border-[#6495ED] rounded-full animate-spin-slow"></div>
                 <span className="text-xs tracking-widest blink">AWAITING INPUT... SCAN SCENE FOR TRACE DATA</span>
               </div>
            ) : (
               collectedEvidence.slice().reverse().map((e, idx) => (
                 <div key={e.id} className="relative pl-6 animate-in slide-in-from-bottom-2 fade-in duration-300">
                    ...
                    <div className="bg-[#151e2e] border-l-2 border-[#6495ED] p-3 shadow-lg hover:bg-[#1e2a40] transition-colors group">
                       <div className="flex flex-col md:flex-row md:justify-between md:items-baseline mb-2 border-b border-[#6495ED]/10 pb-2">
                          <span className="text-[#87CEFA] font-bold text-sm tracking-wide">
                            &gt; {e.name.toUpperCase()}
                          </span>
                          <span className="text-[10px] opacity-50 font-mono mt-1 md:mt-0 group-hover:opacity-80 transition-opacity">
